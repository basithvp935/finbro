...
const AccountManagement: React.FC = () => {
  ...
  const getNextCode = (type: AccountType) => {
    const baseCodes: Record<AccountType, number> = {
      Asset: 1100,
      Liability: 2100,
      Equity: 3100,
      Income: 4100,
    ...
    if (typeAccounts.length === 0) return baseCodes[type].toString();

    const codes = typeAccounts.map(a => parseInt(a.code.replace(/\D/g, ''), 10)).filter(c => !isNaN(c));
    if (codes.length === 0) return baseCodes[type].toString();
  ...
  useEffect(() => {
    if (showAddModal && !editingAccountId) {
      const currentCode = parseInt(newAcc.code);
      const isAutoGenerated = !newAcc.code || [1,2,3,4,5,6,7].includes(Math.floor(currentCode / 1000));
      
      if (isAutoGenerated) {
         const nextCode = getNextCode(newAcc.type);
         if (newAcc.code !== nextCode) {
           setNewAcc(prev => ({ ...prev, code: nextCode }));
         }
      }
    }
  }, [newAcc.type, showAddModal, editingAccountId]);
  ...
  const handleCreateAccount = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAcc.name || !newAcc.code) return showAlert("Validation Protocol", "All identification fields are required to commit this ledger entry.", "error");
    ...
    showAlert(editingAccountId ? "Ledger Updated" : "Ledger Committed", `Account ${newAcc.name} has been successfully synchronized.`, "success");
  };
